...
{	
	...
	import hbworks.whats.Scrolling;
	...
	public final class Xinger extends Sprite
	{
		...
		private var _scroll1:Scrolling = null;
...
*/
			...
			_tf1.addEventListener(Event.SCROLL,
				function(event:Event):void
				{
					var t_r:Number;
					
					if (!_scroll1.get_isScrollDown())
					{
						if (_tf1.maxScrollV > 1)
						{
							t_r = (1 - _tf1.scrollV) / (1 - _tf1.maxScrollV);
							_scroll1.set_ratio(t_r);
							_scroll1.set_enabled(true);
						}
						else
						{
							_scroll1.set_ratio(0);
							_scroll1.set_enabled(false);
						}
					}
					/*
					if (!_scroll2.get_isScrollDown())
					{
						if (_tf1.maxScrollH > 1)
						{
							t_r = (1 - _tf1.scrollH) / (1 - _tf1.maxScrollH);
							_scroll2.set_ratio(t_r);
							_scroll2.set_enabled(true);
						}
						else
						{
							_scroll2.set_ratio(0);
							_scroll2.set_enabled(false);
						}
					}*/
				}
			);
			_scroll1 = new Scrolling(this.scrollBar_1);
			_scroll1.onCallBack = function(eObj:Object):void
			{
				switch (eObj.type)
				{
					case Scrolling.EVENT_TYPE_MOUSE_UP:
					{
						break;
					}
					case Scrolling.EVENT_TYPE_MOUSE_DOWN:
					{
						break;
					}
					case Scrolling.EVENT_TYPE_UPDATE:
					{						
						var t_msv:int = _tf1.maxScrollV;				
						if (t_msv > 1)
						{
							var t_rv:Number = _scroll1.get_ratio();
							var t_sv:Number = Math.round((t_msv - 1) * t_rv);
							_tf1.scrollV = int(t_sv + 1);
						}				
						break;
					}			
					
					case Scrolling.EVENT_TYPE_SCROLL_UP:
					{
						if (_tf1.scrollV > 1)
							_tf1.scrollV -= 1;
						break;
					}
					case Scrolling.EVENT_TYPE_SCROLL_DOWN:
					{
						if (_tf1.scrollV < _tf1.maxScrollV)
							_tf1.scrollV += 1;
						break;
					}
				}
